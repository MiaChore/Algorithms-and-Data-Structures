...
bool namesErrorCompiler(char inputName[], char outputName[]) {
    ...
    if (inputName[i] == outputName[i]) {
        printf("Ошибка: названия файлов должны отличаться\n");
    ...
    if (errorCheck) {
        ...
        printf("Ошибка: работа завершена. Исправь ошибки и запусти заново, ну ёма\n");
        return errorCheck;
    }
}
    ...
    while (fgets(partName, maxName, inputFile) != NULL) {
        ...
        if (part == NULL) {
            ...
            if (ferror(outputFile)) {
                printf("Ошибка записи результата. Работа прервана.\n");
                fclose(part);
                return false;
            }
            if (ferror(part)) {
